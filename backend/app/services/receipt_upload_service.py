...
class ReceiptUploadService:
    def __init__(self, db_session: Session, object_storage: ObjectStorage) -> None:
    ...
    def upload(
        ...
        *,
        ...
        try:
            ...
            self._rollback()
            logger.exception("Failed to create receipt %s", receipt_id)
        ...
        try:
            ...
            logger.exception(
                "Failed to upload image for receipt %s",
                receipt_id,
            )
            raise ReceiptUploadUnavailableError() from error
        ...
        try:
            ...
            self._rollback()
            self._delete_object(object_key)
            logger.exception("Database rejected receipt %s", receipt_id)
            raise ReceiptUploadUnavailableError() from error
            ...
            logger.exception(
                "Commit failed for receipt %s; check database and object %s",
                receipt_id,
                object_key,
            )
            raise ReceiptUploadUnavailableError() from error
